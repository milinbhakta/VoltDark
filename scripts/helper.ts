import { colors, VoltThemes } from './colors'
...
export function createThemeHelpers({ color, soft = false, black = false }: GetThemeOptions) {
  ...
  const v = (key: keyof typeof VoltThemes, op = '') => {
    let obj = black
      ? VoltThemes[`black${capitalize(key)}` as keyof typeof VoltThemes] || VoltThemes[key]
      : soft
        ? VoltThemes[`soft${capitalize(key)}` as keyof typeof VoltThemes] || VoltThemes[key]
        : VoltThemes[key]
